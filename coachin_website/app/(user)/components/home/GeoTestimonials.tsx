...
const GeoTestimonials = () => {
  ...
  // Duplicate the list so the marquee loop is seamless (translateX(-50%) lands exactly
  // at the start of the second copy). Only duplicate once we actually have data.
  const loopTestimonials = useMemo(() => {
    if (displayedTestimonials.length === 0) return [];
    return [...displayedTestimonials, ...displayedTestimonials];
  }, [displayedTestimonials]);

  // Pause the marquee whenever the user is hovering the strip OR has a card
  // expanded (mobile tap), so nothing scrolls out from under them mid-read.
  const shouldPause = isPaused || activeCard !== null;

  const renderCard = (topper: any, cardId: string | number, keySuffix: string) => {
    const isLocal = userCity && topper.city && topper.city.toLowerCase() === userCity.toLowerCase();

    return (
      <div
        key={`${cardId}-${keySuffix}`}
        onClick={() => setActiveCard(activeCard === cardId ? null : cardId)}
        className={`group relative cursor-pointer bg-base-content rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden h-72 w-72 md:w-80 shrink-0 border border-base-200 flex flex-col ${isLocal ? 'ring-2 ring-primary ring-offset-2' : ''}`}
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 flex flex-col justify-end z-10 transform group-hover:translate-y-full transition-transform duration-300">
          ...
          <div className="lg:hidden mt-3">
            <p className="text-xs text-white/90 italic line-clamp-2">
              &quot;{topper.message || topper.quote}&quot;
            </p>
            <p className="text-[10px] text-white/70 font-bold mt-1.5 uppercase tracking-widest flex items-center gap-1">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3 h-3"><path strokeLinecap="round" strokeLinejoin="round" d="M15.042 21.672L13.684 16.6m0 0l-2.51 2.225.569-9.47 5.227 7.917-3.286-.672zM12 2.25V4.5m5.834.166l-1.591 1.591M20.25 10.5H18M7.757 14.743l-1.59 1.59M6 10.5H3.75m4.007-4.243l-1.59-1.59" /></svg>
              Tap to read more
            </p>
          </div>
        </div>
  ...
  return (
    <div className="container mx-auto px-4 max-w-7xl py-4 md:py-6 font-sans">
      ...
      {!isLoading && (
        ...
        >
          {/* Edge fades so cards don't feel like they're getting cut off mid-scroll */}
          <div className="pointer-events-none absolute inset-y-0 left-0 w-12 md:w-24 z-30 bg-gradient-to-r from-base-100 to-transparent" />
          <div className="pointer-events-none absolute inset-y-0 right-0 w-12 md:w-24 z-30 bg-gradient-to-l from-base-100 to-transparent" />

          <div className={`testimonial-track flex gap-6 ${shouldPause ? 'paused' : ''}`}>
            {loopTestimonials.map((topper, index) =>
              renderCard(topper, topper._id || index, index < displayedTestimonials.length ? 'a' : 'b')
            )}
          </div>
        </div>
      )}
