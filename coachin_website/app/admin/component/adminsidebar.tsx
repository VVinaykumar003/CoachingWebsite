...
//       <div className="px-2 pb-2 hidden md:flex items-center justify-start gap-2">
//         <Link href="/admin" className="btn btn-ghost p-0 h-auto hover:bg-transparent hover:border-none hover:shadow-none">
//           <img src="/kunal_logo_ee1.png" alt="Eunoia Education Logo" className="h-13 md:h-16 w-auto max-w-full object-contain" />
//         </Link>
//       </div>
//       <hr className='w-full '/>
...
const AdminSidebar = () => {
    ...
    <li>
      <Link
        ...
        className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors duration-150
          ${isActive(href)
            ? 'bg-brand-gradient text-base-200 font-medium'
            : 'text-base-200/50 hover:bg-brand-gradient hover:text-base-200/85'
          }`}
      >
        <i className={`ti ${icon} text-[17px] w-5 text-center shrink-0`} aria-hidden="true" />
        {label}
        {isActive(href) && (
  ...
  return (
    <aside className="min-h-screen w-64 bg-base-content flex flex-col p-3 border-r border-white/[0.07] shrink-0">

      {/* Logo */}
      <div className="flex items-center gap-2.5 px-2 pb-4   " >
        {/* <div className="w-9 h-9 rounded-[10px] bg-gradient-to-br from-indigo-500 to-violet-500 flex items-center justify-center text-white">
          <i className="ti ti-building-school text-lg" aria-hidden="true" />
        </div>
        <div>
          <p className="text-sm font-medium text-base-100 leading-none">Eunoia</p>
          <p className="text-[11px] text-base-100/40 mt-0.5">Admin Panel</p>
        </div> */}
        <Image
        width={250}
        height={100}
        src="/kunal_logo_ee1.png"
        alt="Eunoia Education Logo"
        className="h-13 md:h-15 w-full max-w-full object-contain "
        />
      </div>

      {/* <hr className="border-white/[0.08] mx-2 mb-4" /> */}

      {/* Nav */}
      <p className="text-[10px] uppercase tracking-widest text-base-100/50 font-medium px-3 mb-1.5">
        Admin Menu
      </p>
      <ul className="flex flex-col gap-0.5 flex-1">
        {navLink('/admin',               'ti-layout-dashboard', 'Dashboard')}
        {navLink('/admin/managecourses', 'ti-books',            'Manage Courses')}
        {navLink('/admin/manageblogs',   'ti-article',          'Manage Blogs')}
        {navLink('/admin/managetestimonial', 'ti-star',         'Manage Testimonials')}
        {navLink('/admin/managestudent', 'ti-address-book' ,         'Manage Students')}
        {navLink('/admin/managebatches', 'ti-layout-grid' ,         'Manage Batches')}
      </ul>

      {/* Footer */}
      <div className="border-t border-base-100/8 pt-3 mt-2">
        <button
          onClick={() => { localStorage.removeItem('adminToken'); router.push('/login'); }}
          className="flex items-start gap-2.5 w-full  py-2.5 rounded-lg text-sm  transition-colors btn btn-outline btn-error hover:bg-transparent hover:border-error hover:shadow-none"
        >
          {/* <i className="ti ti-logout text-[17px] w-5 text-center bg-black" aria-hidden="true" 
          /> */}
          Logout
        </button>
        <p className="text-[11px] text-center text-base-100/68 mt-3">
          © {new Date().getFullYear()} Eunoia Education
        </p>
      </div>
